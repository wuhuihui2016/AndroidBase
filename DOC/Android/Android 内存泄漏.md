...
二、原因分析及解决办法
  ...
  2、webView泄露，Dialog泄漏
   解决办法：在onDestory()方法中释放引用
    ...
    mWebView.destroy();
    Process.killProcess(Process.myPid()); //一线企业处理方法
...
四、内存泄漏和内存溢出的区别
  ...
  内存溢出(out of memory)是指程序在申请内存时，没有足够的内存空间供其使用。
      原因：1、Java 堆内存溢出；2、无足够连续内存空间；3、FD 数量超出限制；4、线程数量超出限制(操作系统(OS)限制线程数：Linux 100个，windows 200个)；5、虚拟内存不足
  内存抖动：指程序短时间内大量创建对象，然后回收的现象

五、垃圾回收 【https://www.jianshu.com/p/214e42fc0d37】
   引用记数是一种简单但速度很慢的垃圾回收技术。每个对象都含有一个引用记数器，当有引用连接至对象时，引用计数加1。
   当引用离开作用域或被置为null时，引用计数减1。虽然管理引用记数的开销不大，但这项开销在整个程序生命周期中将持续发生。
   垃圾回收器会在含有全部对象的列表上遍历，当发现某个对象的引用计数为0时，就释放其占用的空间（但是，引用记数模式经常会在记数值变为0时立即释放对象)。
